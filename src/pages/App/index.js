...
const TOKEN =
  "pk.eyJ1IjoiZmFiaW9raWRvIiwiYSI6ImNrMzliNjUwOTAxODMzZm1oem1jY2VnaWgifQ.Ux1dkHF4clQMrOlKZNTL7Q";
...
class Map extends Component {
  ...
  loadPositionUser = async () => {
    const geo = navigator.geolocation; 

    if(geo){
      const geo = navigator.geolocation.watchPosition((position) => {
        const { latitude, longitude } = position.coords;
        this.setState({ viewport : { latitude: latitude, longitude: longitude, zoom: 12.8, bearing: 0, pitch: 0 } });
      });
    }else{
      console.log('Geolocation não permitido!');
    }  
  }
